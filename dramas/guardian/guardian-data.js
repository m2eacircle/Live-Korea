...
var GUARDIAN_DATA = {
  ...
  titleKr: '쓸쓸하고 찬란하神 – 도깨비',
  emoji: '🌊',
  totalEpisodes: 16,
  shortsPerEpisode: 30,
  genreEn: 'Fantasy Romance',
  genreKr: '판타지 로맨스',

  /* 30 Korean expressions with English translations */
  expressions: [
  ...
  /*
   * VIDEO MAP — fill in YouTube Short IDs as you upload them.
   * Key format: "ep{episode}_{expr_id}"
   * e.g. videos['ep1_1'] = 'dQw4w9WgXcQ'  (short ID for ep1, expr #1)
   * Leave as null if not yet uploaded.
   */
  videos: {}
};

/* Helper to get video ID for a given episode + expression */
function getGuardianVideo(episode, exprId) {
  var key = 'ep' + episode + '_' + exprId;
  return GUARDIAN_DATA.videos[key] || null;
}
